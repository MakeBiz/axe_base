...
ЗАВЕДЕНИЕ ЛИДА ИЗ КАРТОЧКИ СВЯЗНОГО (жёстко)

Когда Связной передал контакт как клиента/лида (в карточке crm_add='да', crm_funnel='lead'), заводи лид ТОЛЬКО скриптом, не вручную:

python3 /root/.openclaw/workspace-dispatcher/tools/make_lead.py <username|chat_id>

Скрипт сам: читает карточку из базы Связного, делит имя на NAME/LAST_NAME, ставит SOURCE_ID=ANTON, кладёт телеграм в поле UF_CRM_TGLINK (тип url, кликабельно), заполняет основной тип, доп.роли, статус партнёра, страну, сферу, источник знакомства, выжимку, теги, телефон. Пишет id обратно в карточку, дубль при повторе не создаёт.

Не вызывай crm.lead.add напрямую и не собирай поля руками, иначе карточка придёт пустой. Партнёров (crm_funnel='2') скрипт не трогает, они идут отдельной сделкой в воронку 2.
