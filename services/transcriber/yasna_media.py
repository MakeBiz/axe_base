# yasna_media.py — изолированный медиа-обработчик группы Златы, только в память Ясны
...
def register(client, oai, transcribe, pg_insert):
 def _distill(text):
  try:
   r=oai.chat.completions.create(model='gpt-4o-mini',temperature=0,messages=[
    {'role':'system','content':'Ты ведёшь память бизнес-коуча о Злате. Из расшифровки вытащи только устойчивые факты о Злате, её методе и экспертизе, продукте, аудитории, договорённостях и целях по монетизации. От 1 до 6 пунктов, каждый одной короткой строкой, без нумерации и маркеров. Если ничего стоящего нет, верни пусто.'},
 ...
 async def yasna_media(ev):
  ...
  try:
   ...
   traceback.print_exc()
 print('YASNA_MEDIA registered for', YASNA_GROUP)
 return True
